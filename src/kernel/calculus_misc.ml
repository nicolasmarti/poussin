...
(* some name freshness *)
(* poussin refresh: now string are immutable *)
let rec add_string_index (y: string) index =
  let len = (String.length y - index) in
  match (String.get y) len with 
  | c when c >= '0' && c < '9' -> (String.sub y 0 len) ^ (String.make 1 (char_of_int (int_of_char c + 1)))
  | '9' -> (add_string_index (y: string) (index + 1)) ^ (String.make 1 '0')
  | c -> (String.concat "" (y :: "0" :: [])) ;;
